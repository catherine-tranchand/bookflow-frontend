...
// ─── addBook ──────────────────────────────────────────────────────────────────
export const addBook = async ({ title, author, description, image, creator, genre, state, type }) => {
    try {
        const book = await databases.createDocument(
            databaseId,
            booksCollectionId,
            ID.unique(),
            { title, author, description, image, creator, genre, state, type }
        );
...
export const uploadImage = async (uri) => {
    try {
        ...
        const file = {
            ...
            name: fileName,
            type: 'image/jpeg',
            size: 1000, // ✅ ajoute une taille approximative
        };
        ...
        const imageUrl = `https://cloud.appwrite.io/v1/storage/buckets/${storageId}/files/${uploadedFile.$id}/view?project=${projectId}`;
        return imageUrl;
